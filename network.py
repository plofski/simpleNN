...
class Perceptron:
    def __init__(self,units,activation_function,input_shape=None,weights=None,bias=None,name=None):
    ...
    def build(self, input_shape):
        ...
        if self.weights is None:
            self.weights = np.random.uniform(low=-1,high=1,size=(self.input_shape,self.units))
    ...
    def forward_pass(self,input_vector):
        ...
        return self.activation_function.forward_pass(np.dot(input_vector,self.weights)+self.bias)
...
class MLP:
    ...
    def backward_pass(self, input, output, target, hidden_outputs, learning_rate=0.01):
        ...
        for layer_ind in reversed(range(len(self.layers) - 1)):
            ...
            if layer_ind > 0:
                hidden_layer.backward_pass(in_deltas, hidden_outputs[layer_ind - 1], learning_rate)
            else:
    ...
    def fit(self, epochs, x_train, y_train, learning_rate=0.01):
        for i in range(epochs):
            ...
            for input_vector, target in zip(x_train, y_train):
                ...
                epoch_loss += loss
                self.backward_pass(input_vector.reshape(1, -1), output, target.reshape(1, -1), hidden_outs, learning_rate)
    ...
    def accuracy(self,x,y):
        ...
        # accuracy
        correct_predictions = sum(1 for pred, true in zip(predictions, training_targets.flatten()) if pred == true)
...
if __name__ == "__main__" :
    ...
    for input, target in zip(training_inputs, training_targets):
        ...
        # Optionally, print the predicted value for each iteration
